...
local GetGroupMembers = function(reversed, forceParty)
    ...
    local numGroupMembers =
        (forceParty and _G.GetNumSubgroupMembers() or _G.GetNumGroupMembers()) - (unit == "party" and 1 or 0)
...
function SAP:UpdateGUI(t)
    ...
    local sortedKeys = {}
    for k, v in pairs(t) do table.insert(sortedKeys, k) end
    ...
    for _, k in ipairs(sortedKeys) do
        ...
        table.sort(t[k], function(a, b) return a:upper() < b:upper() end)
    ...
    for unit in GetGroupMembers() do
        ...
        for i = 1, MAW_BUFF_MAX_DISPLAY do
            local spellName,
                ...
                isStealable,
                shouldConsolidate,
                spellId = _G.UnitAura(unit, i, "MAW")
